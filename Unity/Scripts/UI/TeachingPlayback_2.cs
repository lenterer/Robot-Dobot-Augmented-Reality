...
public class TeachingPlayback_2 : MonoBehaviour
{
    [Header("Data")]
    public List<Waypoint_2> waypoints = new List<Waypoint_2>();
    ...
    public GameObject markerPrefabOn;
    public GameObject markerPrefabOff;
    ...
    public IKButtonController_2 ikController;
    ...
    // =========================
    public void AddWaypointFromTarget(bool isSuctionEvent = false)
    {
       if (targetSource1 == null || targetSource2 == null || robotBase == null)
        ...
        float j1 = jointController.realTarget1;
        float j2 = jointController.realTarget2;
        float j3 = jointController.realTarget3;

        AddWaypoint(visualRel, robotRel, j1, j2, j3, isSuctionEvent);
    }
    ...
    // ADD WAYPOINT (MANUAL)
    // =========================
    public void AddWaypoint(Vector3 visualPos, Vector3 robotPos, float j1, float j2, float j3, bool isSuctionEvent = false)
    {
        Waypoint_2 wp = new Waypoint_2();
        wp.visualPosition = visualPos;
        wp.robotPosition = robotPos;
        wp.r = Mathf.Round(joint4 * 100f) / 100f;

        // simpan joint
        wp.joint1 = Mathf.Round(j1 * 100f) / 100f;
        wp.joint2 = Mathf.Round(j2 * 100f) / 100f;
        wp.joint3 = Mathf.Round(j3 * 100f) / 100f;

        wp.suctionOn = currentSuctionState;

        waypoints.Add(wp);

        Transform parent = targetSource1 != null ? robotBase : markerParent;

        Vector3 worldVisual = robotBase.TransformPoint(visualPos);
        GameObject selectedPrefab = markerPrefab; // Default: marker biasa

        if (isSuctionEvent)
        {
            // Jika dipicu tombol suction, pilih antara prefab ON atau OFF
            selectedPrefab = currentSuctionState ? markerPrefabOn : markerPrefabOff;
        }

        // Spawn menggunakan prefab yang terpilih
        GameObject marker = Instantiate(selectedPrefab, worldVisual, Quaternion.identity, parent);
        markers.Add(marker);
    ...
    {
        currentSuctionState= true;
        string customJson = "{\"suction\":true}";
        if (wsClient != null && Teleop.isOn)
        {
            wsClient.SendPlayerData(customJson);
        }

        Debug.Log("Suction: ON");
        AddWaypointFromTarget(true);
    }

    public void SuctionOff()
    {
        currentSuctionState= false;
        string customJson = "{\"suction\":false}";
        if (wsClient != null && Teleop.isOn)
        {
            wsClient.SendPlayerData(customJson);
        }
    ...
    public void SetTarget(Transform target1, Transform target2, Transform target3, MultiJointControl target4)
    {
